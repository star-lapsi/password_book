...
int data_operate(void)
{
    ...
    printf("6 - Encrypt a password in database.\n");
    printf("7 - Delete one password from database.\n");
    printf("8 - Save all password in database to files.\n");
    printf("0 - Back to database operating.\n");
...
{
    int flag_count;
    int row,row_tail,flag_row;
    row = count_ls / PAGE_SIZE;
    row_tail = count_ls % PAGE_SIZE;
    const char *list_head[7]={"No.","Username","Domain","Prompt","Telephone","Crypt"};
    if(count_ls!=0)
    {
        printf("There is already %d data in memory.\n",count_ls);
        for(flag_row = 0 ; flag_row < row ; flag_row++)
        {
            printf("-----------------------------------------------------------------------------\n");
            printf("%-5.3s%-15.12s%-20.15s%-20.15s%-15.13s%-5.5s\n",list_head[0],list_head[1],list_head[2],list_head[3],list_head[4],list_head[5]);
            printf("\n");
            for(flag_count = flag_row * PAGE_SIZE ; flag_count < (flag_row + 1) * PAGE_SIZE ; flag_count++,mem_ls++)
                printf("%-5d%-15.12s%-20.15s%-20.15s%-15.13s%-5.2c\n",mem_ls->xuhao+1,mem_ls->username,mem_ls->domains,mem_ls->prompt,mem_ls->telephone,mem_ls->crypt);
            printf("-----------------------------------------------------------------------------\n");
            standing_by();
            system("cls");
        }
        if(row_tail!=0)
        {
            ...
            printf("%-5.3s%-15.12s%-20.15s%-20.15s%-15.13s%-5.5s\n",list_head[0],list_head[1],list_head[2],list_head[3],list_head[4],list_head[5]);
            printf("\n");
            for(flag_count = flag_row * PAGE_SIZE ; flag_count < flag_row * PAGE_SIZE + row_tail ; flag_count++,mem_ls++)
                printf("%-5d%-15.12s%-20.15s%-20.15s%-15.13s%-5.2c\n",mem_ls->xuhao+1,mem_ls->username,mem_ls->domains,mem_ls->prompt,mem_ls->telephone,mem_ls->crypt);
            printf("-----------------------------------------------------------------------------\n");
            standing_by();
            system("cls");
